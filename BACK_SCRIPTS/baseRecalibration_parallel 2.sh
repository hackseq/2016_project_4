...
gatk -T PrintReads -R ${REFERENCE_FASTA} -I ${bam} -o ${filename}_reassigned.bam -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 --defaultBaseQualities 60 -U ALLOW_N_CIGAR_READS	
# Base quality score recalibration
echo "Generating calibration table.."
gatk -T BaseRecalibrator -R ${REFERENCE_FASTA} -I ${filename}_reassigned.bam -knownSites ${KNOWN_SITES} -o ${filename}_reassigned_recal_data.table -U ALLOW_N_CIGAR_READS

# Print reads 
echo "Generating re-calibrated bam.."
gatk  -T PrintReads -R ${REFERENCE_FASTA} -I ${filename}_reassigned.bam -BQSR ${filename}_reassigned_recal_data.table -o ${filename}_recalibrated.bam -U ALLOW_N_CIGAR_READS
